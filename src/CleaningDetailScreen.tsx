...
const CleaningDetailScreen: React.FC = () => {
  ...
  const fetchCleaningDetails = async () => {
    try {
      const cleaningDoc = await getDoc(doc(db, 'cleaningJobs', cleaningJobId));
      if (cleaningDoc.exists()) {
        const cleaningData = { id: cleaningDoc.id, ...cleaningDoc.data() } as CleaningJob;
        console.log('🔍 Cleaning job data from Firebase:', cleaningData);
  ...
  const formatDate = (timestamp: number | string | undefined | null) => {
    ...
    if (!timestamp && timestamp !== 0) {
      console.log('Date is undefined or null:', timestamp);
          ...
          <View style={styles.content}>
            ...
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                ...
                <Text style={styles.cardTitle}>Cleaner Assignment</Text>
              </View>
              <View style={styles.cardContent}>
                {cleaning.cleanerFirstName ? (
                  <Text style={styles.cleanerName}>
                    {cleaning.cleanerFirstName} {cleaning.cleanerLastName}
                  </Text>
                ) : (
                  <>
                    <Text style={styles.unassignedText}>No cleaner assigned</Text>
                    <TouchableOpacity style={styles.assignButton} onPress={handleAssignCleaner}>
                      <Text style={styles.assignButtonText}>Assign Cleaner</Text>
                    </TouchableOpacity>
                  </>
                )}
              </View>
