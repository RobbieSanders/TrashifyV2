...
// Define CleaningBid interface for this component
...
// Extend CleaningJob to include bids property for this component
  ...
  useEffect(() => {
    ...
    const unsubOpen = onSnapshot(
      ...
      (snapshot) => {
        ...
        jobs.forEach(job => {
          const jobWithBids = job as CleaningJobWithBids;
          const myBid = jobWithBids.bids?.find((b: CleaningBid) => b.cleanerId === user.uid);
          if (myBid) {
            bidsWithJobs.push({ jobId: job.id, job: jobWithBids, bid: myBid });
          }
        });
