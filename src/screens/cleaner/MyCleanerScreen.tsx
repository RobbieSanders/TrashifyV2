...
// Local type definitions for bidding functionality
...
interface CleaningJobWithBids extends CleaningJob {
  bids?: CleaningBid[];
}

export function MyCleanerScreen({ navigation, route }: any) {
  const user = useAuthStore(s => s.user);
  const { width, height } = useWindowDimensions();
  const isTwoPane = width >= 900 || (width >= 700 && width > height);
  
  // Determine which tab we're on
  const isMyCleans = route?.name === 'CleanerJobs' || route?.name === 'CleanerActive';
  
  const [cleaningJobs, setCleaningJobs] = useState<CleaningJobWithBids[]>([]);
  const [myActiveJobs, setMyActiveJobs] = useState<CleaningJob[]>([]);
  const [myBids, setMyBids] = useState<{ jobId: string; job: CleaningJobWithBids; bid: CleaningBid }[]>([]);
  const [showBidModal, setShowBidModal] = useState(false);
  const [selectedJob, setSelectedJob] = useState<CleaningJobWithBids | null>(null);
  ...
  useEffect(() => {
    ...
    // Subscribe to open cleaning jobs for bidding
    ...
    const unsubOpen = onSnapshot(
      openJobsQuery, 
      (snapshot) => {
        const jobs: CleaningJobWithBids[] = [];
        snapshot.forEach((doc) => {
          const job = { id: doc.id, ...doc.data() } as CleaningJobWithBids;
          jobs.push(job);
        });
        setCleaningJobs(jobs);
        
        // Extract bids for this cleaner
        const bidsWithJobs: { jobId: string; job: CleaningJobWithBids; bid: CleaningBid }[] = [];
        jobs.forEach(job => {
          const myBid = job.bids?.find((b: any) => b.cleanerId === user.uid);
          if (myBid) {
    ...
    // Subscribe to cleaner's active jobs
              ...
              {myActiveJobs.map(job => (
                job.destination && (
                  <Marker
                    ...
                    title={job.address}
                    pinColor={job.status === 'assigned' ? '#3B82F6' : '#10B981'}
                  />
                )
              ))}
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {cleaningJobs.length === 0 ? (
        <Text style={styles.muted}>No available jobs at the moment</Text>
      ) : (
        cleaningJobs.filter(j => !myBids.find(b => b.jobId === j.id)).map(job => (
          <View key={job.id} style={styles.card}>
            ...
            {job.bids && job.bids.length > 0 && (
              <Text style={[styles.muted, { marginTop: 4, color: '#3B82F6' }]}>
                {job.bids.length} bid{job.bids.length > 1 ? 's' : ''} already placed
              </Text>
            )}
