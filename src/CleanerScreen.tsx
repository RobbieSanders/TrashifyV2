...
export function CleanerScreen({ navigation, route }: any) {
  ...
  useEffect(() => {
    ...
    const unsubOpen = onSnapshot(
      ...
      (snapshot) => {
        ...
        console.log('[CleanerScreen] Loaded cleaning jobs:', jobs.length);
    ...
    // Query for active jobs assigned to this cleaner
    ...
    const unsubActive = onSnapshot(
      ...
      (snapshot) => {
        ...
        jobs.sort((a, b) => (a.cleanerPriority || 0) - (b.cleanerPriority || 0));
        console.log('[CleanerScreen] Loaded active jobs:', jobs.length);
        setMyActiveJobs(jobs);
      },
      (error) => {
        console.error('[CleanerScreen] Error loading active jobs:', error);
        setMyActiveJobs([]);
      }
    );
  ...
  const handlePlaceBid = async () => {
    ...
    try {
      ...
      const existingBidIndex = selectedJob.bids?.findIndex(b => b.cleanerId === user.uid);
      if (existingBidIndex !== undefined && existingBidIndex >= 0) {
        Alert.alert('Already Bid', 'You have already placed a bid on this job');
        return;
      }

      const updatedBids = [...(selectedJob.bids || []), newBid];
      
      console.log('[CleanerScreen] Submitting bid:', {
      ...
      // Update the document with proper status
      await updateDoc(jobRef, { 
        bids: updatedBids,
        status: 'bidding'  // Ensure status is set to bidding when bids are placed
      });

      console.log('[CleanerScreen] Bid submitted successfully');
      ...
      console.error('[CleanerScreen] Error submitting bid:', error);
